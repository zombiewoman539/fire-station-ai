...
export function IncomeSection({ inputs, onChange }: { inputs: FireInputs; onChange: (i: FireInputs) => void }) {
  ...
  const retItems = inputs.income.retirementExpenseItems ?? [];

  const itemsTotal = (list: ExpenseLineItem[]) =>
    list.reduce((s, e) => s + (e.frequency === 'monthly' ? e.amount * 12 : e.amount), 0);

  const addItem = () => {
    const newItem: ExpenseLineItem = { id: uid(), label: 'New expense', amount: 0, frequency: 'monthly', category: 'variable' };
    const next = [...items, newItem];
    onChange({ ...inputs, income: { ...inputs.income, expenseItems: next, annualExpenses: itemsTotal(next) } });
  };
  const removeItem = (id: string) => {
    const next = items.filter(e => e.id !== id);
    const total = itemsTotal(next);
  ...
  // Retirement-expense breakdown — mirrors accumulation pattern
  const addRetItem = () => {
    const newItem: ExpenseLineItem = { id: uid(), label: 'New retirement expense', amount: 0, frequency: 'monthly', category: 'variable' };
    const next = [...retItems, newItem];
    onChange({ ...inputs, income: { ...inputs.income, retirementExpenseItems: next, retirementExpenses: itemsTotal(next) } });
  };
  const removeRetItem = (id: string) => {
    const next = retItems.filter(e => e.id !== id);
    const total = itemsTotal(next);
    onChange({ ...inputs, income: { ...inputs.income, retirementExpenseItems: next, retirementExpenses: total || inputs.income.retirementExpenses } });
  };
  const updateRetItem = (id: string, patch: Partial<ExpenseLineItem>) => {
    const next = retItems.map(e => e.id === id ? { ...e, ...patch } : e);
    onChange({ ...inputs, income: { ...inputs.income, retirementExpenseItems: next, retirementExpenses: itemsTotal(next) } });
  };
  const expandRetExpenses = () => {
    const monthly = Math.round(inputs.income.retirementExpenses / 12);
    const seed: ExpenseLineItem = { id: uid(), label: 'Living expenses', amount: monthly, frequency: 'monthly', category: 'variable' };
    onChange({ ...inputs, income: { ...inputs.income, retirementExpenseItems: [seed] } });
  };
  const collapseRetExpenses = () => {
    const total = itemsTotal(retItems);
    onChange({ ...inputs, income: { ...inputs.income, retirementExpenseItems: undefined, retirementExpenses: total || inputs.income.retirementExpenses } });
  };
      ...
      <div style={{ borderTop: '1px solid var(--border-soft)', paddingTop: 16, marginTop: 8 }}>
        ...
        {retItems.length === 0 ? (
          <NumberField label="Retirement Expenses / Year" value={inputs.income.retirementExpenses} prefix="S$"
            ...
            rightSlot={<DetailToggleButton icon="☰" label="Break into line items" onClick={expandRetExpenses} />}
            onChange={v => upd('retirementExpenses', v)} />
        ) : (
          <div style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <SectionLabel>Retirement Expense Breakdown</SectionLabel>
              <DetailToggleButton icon="↑" label="Use single total" onClick={collapseRetExpenses} />
            </div>
            {retItems.map(item => {
              const annualEquiv = item.frequency === 'monthly' ? item.amount * 12 : item.amount;
              return (
                <div key={item.id} style={{ background: 'var(--inset)', borderRadius: 10, padding: '10px 12px', marginBottom: 8, border: '1px solid var(--border-soft)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <input value={item.label} onChange={e => updateRetItem(item.id, { label: e.target.value })}
                      placeholder="e.g. Housing, Healthcare, Travel"
                      style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', color: 'var(--text-1)', fontSize: 13, fontWeight: 600 }} />
                    <button onClick={() => removeRetItem(item.id)}
                      style={{ color: 'var(--text-5)', fontSize: 13, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>✕</button>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', background: 'var(--input-bg)', border: '1px solid var(--input-border)', borderRadius: 8, padding: '6px 10px' }}>
                      <span style={{ color: 'var(--text-4)', fontSize: 12, marginRight: 4 }}>S$</span>
                      <input type="number" value={item.amount} onChange={e => updateRetItem(item.id, { amount: Number(e.target.value) || 0 })}
                        style={{ background: 'transparent', border: 'none', outline: 'none', color: 'var(--text-1)', fontSize: 13, width: '100%' }} />
                    </div>
                    <div style={{ display: 'flex', borderRadius: 8, overflow: 'hidden', border: '1px solid var(--border)', flexShrink: 0 }}>
                      {(['monthly', 'annual'] as const).map(f => (
                        <button key={f} onClick={() => updateRetItem(item.id, { frequency: f })} style={{
                          fontSize: 11, padding: '6px 8px', border: 'none', cursor: 'pointer',
                          background: item.frequency === f ? 'var(--border-mid)' : 'var(--input-bg)',
                          color: item.frequency === f ? 'var(--text-1)' : 'var(--text-4)',
                          fontWeight: item.frequency === f ? 700 : 400,
                    ...
                    <div style={{ display: 'flex', borderRadius: 8, overflow: 'hidden', border: '1px solid var(--border)', flexShrink: 0 }}>
                      {(['fixed', 'variable'] as const).map(c => (
                        <button key={c} onClick={() => updateRetItem(item.id, { category: c })} style={{
                          fontSize: 11, padding: '6px 8px', border: 'none', cursor: 'pointer',
                          background: item.category === c ? (c === 'fixed' ? 'rgba(59,130,246,0.2)' : 'rgba(251,191,36,0.15)') : 'var(--input-bg)',
                          color: item.category === c ? (c === 'fixed' ? '#60a5fa' : '#fbbf24') : 'var(--text-4)',
                          fontWeight: item.category === c ? 700 : 400,
            ...
            <button onClick={addRetItem} style={{
              width: '100%', padding: '8px 0', fontSize: 12, color: '#34d399',
              border: '1px dashed rgba(52,211,153,0.4)', borderRadius: 8, background: 'none', cursor: 'pointer', marginBottom: 6,
            }}>+ Add retirement expense</button>
            <div style={{ textAlign: 'right', fontSize: 13, fontWeight: 700, color: 'var(--text-2)' }}>
              Total: S${retTotal.toLocaleString()}/yr
            </div>
          </div>
        )}
      </div>
